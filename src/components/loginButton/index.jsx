...
const LoginButton = () => {
  ...
  return (
	<>
		...
		{
			user.userInfo.islogin == false && 
			<button className = "dropbtn" onClick={()=>setLogInModalShow(true)}>
				Log In
			</button>
		}

		{
			...
			<DropDown title={user.userInfo.userName} avatar={`${SERVER_URL}/useravatar/${user.userInfo.userAvatar}`}>
				<a onClick={user.logout}>Log out</a>
			</DropDown>
		}
